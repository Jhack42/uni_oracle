...
def obtener_informacion_clase():
    ...
    # Ejemplo de consulta para obtener la información de la clase y los correos de los profesores
    cursor.execute("SELECT P.nombres, P.apellidos, P.correo_electronico, SC.fecha_inicio, C.nombre_curso FROM Profesores P INNER JOIN Cursos C ON P.id_profesor = C.id_profesor INNER JOIN Sesiones_Curso SC ON SC.id_curso = C.id_curso")
...
def enviar_notificaciones_toma_clase():
    ...
    informacion_clases = obtener_informacion_clase()
    
    for info_clase in informacion_clases:
        nombre_docente, apellidos_docente, correo_docente, dia_clase, nombre_curso = info_clase

        # Contenido del correo sin comillas alrededor del nombre del curso
        mensaje = f"Estimado {nombre_docente},\n\nSe le hace recordar que el día {dia_clase} tiene sesión del curso {nombre_curso}.\n\n<Texto argumentativo>"
        ...
        try:
            ...
            servidor.quit()
            print(f"Correo de notificación enviado exitosamente a {correo_docente}.")
        except Exception as e:
            print(f"Error al enviar el correo a {correo_docente}: {e}")

# Ejecutar la función para enviar las notificaciones a todos los registros
enviar_notificaciones_toma_clase()
